...
class _AcceptanceSmuPageState extends State<AcceptanceSmuPage> {
  ...
  void _onQRViewCreated(QRViewController controller) {
    ...
    controller.scannedDataStream.listen((scanData) {
      setState(() {
        result = scanData;
        _smuController.text = result!.code.toString();
        controller.pauseCamera();
      });
          ...
          children: [
            ...
            Column(
              ...
              children: [
                ...
                Text(
                  this.widget.data!.nama_pengemudi.toString() +
                      ' (' +
                      this.widget.data!.no_polisi_kendaraan.toString() +
                      ')',
                  textAlign: TextAlign.start,
                  style: TextStyle(
                    fontSize: 16,
                  ...
                  showDialog(
                      ...
                      builder: (BuildContext context) {
                        return Dialog(
                          shape: RoundedRectangleBorder(
                            borderRadius:
                                BorderRadius.circular(Paddings.padding),
                          ),
                          elevation: 0,
                          backgroundColor: Colors.transparent,
                          child: contentBox(context),
                      ...
                      children: [
                        ...
                        Align(
                          ...
                          child: SizedBox(
                            ...
                            child: RaisedButton(
                              onPressed: () async {
                                var selesai = await api
                                    .selesaiSmuAcceptence(widget.data!.id);
                                print(selesai);
                                if (selesai == true) {
                                  Navigator.push(
                                      context,
      ...
      children: <Widget>[
        Container(
          padding: EdgeInsets.only(
              ...
              top: Paddings.avatarRadius + Paddings.padding,
                ...
                child: SingleChildScrollView(
                  child: Column(
                    children: [
                      Row(
                        children: [
                          Expanded(flex: 1, child: _autoCompleteSmu(context)),
                          SizedBox(
                            width: 16,
                          ),
                          Expanded(flex: 1, child: _autoCompleteAgen(context)),
                        ],
                      ),
                      SizedBox(
                        height: 16,
                      ...
                      Row(
                        children: [
                          Expanded(flex: 1, child: _textEditNohp(context)),
                          SizedBox(
                            width: 16,
                          ),
                          Expanded(flex: 1, child: _textEditAlamat(context)),
                        ],
                      ),
                      SizedBox(
                        height: 16,
                      ...
                      Row(
                        children: [
                          Expanded(flex: 1, child: _textEditCompany(context)),
                          SizedBox(
                            width: 16,
                          ),
                          Expanded(
                              flex: 1, child: _autoCompleteBarang(context)),
                        ],
                      )
                    ],
                  ),
        ...
        Positioned(
          ...
          child: CircleAvatar(
            ...
            child: ClipRRect(
                borderRadius:
                    BorderRadius.all(Radius.circular(Paddings.avatarRadius)),
                child: Container(
                  color: Colors.white,
      ...
      children: [
        ...
        Row(
          ...
          children: [
            ...
            Expanded(flex: 1, child: Text('${smu.smu.toString()}')),
            Expanded(flex: 1, child: Text('${smu.koli.toString()}')),
            Expanded(flex: 1, child: Text('${smu.berat_total.toString()} Kg')),
          ],
        )
      ],
    );
  ...
  _autoCompleteSmu(contex) {
    return TypeAheadField(
      textFieldConfiguration: TextFieldConfiguration(
          ...
          style: TextStyle(),
          decoration:
              InputDecoration(border: OutlineInputBorder(), hintText: 'SMU')),
    ...
    return TypeAheadField(
      textFieldConfiguration: TextFieldConfiguration(
          ...
          decoration: InputDecoration(
              border: OutlineInputBorder(), hintText: 'Nama Agen')),
      suggestionsCallback: (pattern) async {
        return await api.getSuggestAgen(pattern);
      },
      itemBuilder: (context, SuggestAgen suggestion) {
        return ListTile(title: Text(suggestion.customer.toString()));
      },
  ...
  _textEditCompany(contex) {
    return TextField(
        ...
        decoration: InputDecoration(
            border: OutlineInputBorder(), hintText: 'Nama Perusahaan'));
  }
  ...
  _textEditNohp(contex) {
    return TextField(
        ...
        keyboardType: TextInputType.text,
        decoration:
            InputDecoration(border: OutlineInputBorder(), hintText: 'No Hp'));
  }
  ...
  _textEditAlamat(contex) {
    return TextField(
        ...
        keyboardType: TextInputType.text,
        decoration:
            InputDecoration(border: OutlineInputBorder(), hintText: 'Alamat'));
  }
